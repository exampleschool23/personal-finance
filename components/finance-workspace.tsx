...
import { Entry, kinds, assets, liabilities, income, expenses, value, monthly, estimatedCashFlow } from '@/lib/finance';
const today = () => new Date().toISOString().slice(0, 10);
const fresh = (): Entry => ({ id: crypto.randomUUID(), name: '', kind: 'Cash', currency: 'USD', amount: 0, quantity: 1, cost: 0, rate: 0, date: today(), lent_date: today(), frequency: 'Once', notes: '', business_id: null, ownership_percentage: 100, estimated_monthly_income: 0 });
const sample = (): Entry[] => [
...
export default function FinanceWorkspace() {
    ...
    useEffect(() => {
        ...
        fetch('/api/records?' + params, { signal: controller.signal }).then(async response => {
            ...
            const normalize = (e: Entry): Entry => ({ ...e, date: e.date ?? '', lent_date: e.lent_date ?? '', amount: Number(e.amount), quantity: Number(e.quantity), cost: Number(e.cost), rate: Number(e.rate), ownership_percentage: Number(e.ownership_percentage ?? 100), estimated_monthly_income: Number(e.estimated_monthly_income ?? 0) });
    ...
    const forecast = estimatedCashFlow(current);
