 ...
 return <>
  ...
  <div className="planning-cards account-cards">
   {balances.map(account=><article className="panel account-card" key={account.id}>
    ...
    <div className="account-card-actions">{account.kind==='Deposit'?<><Button onClick={()=>setMovement({kind:'transfer',target_id:account.id})}><Plus size={16} aria-hidden="true" />{t('Top-up')}</Button><Button variant="outline" onClick={()=>setMovement({kind:'transfer',source_id:account.id})}>{t('Withdraw')}</Button></>:<Button variant="outline" onClick={()=>setOperation({action:'reconcile',account_id:account.id,amount:account.amount})}>{t('Reconcile balance')}</Button>}</div>
   ...
   {investmentAccounts.map(account=>{
    ...
    return <article className="panel account-card" key={account.id}>
     <header><span className="account-card-icon">{account.kind==='Stock'?<ChartNoAxesCombined aria-hidden="true" />:<Bitcoin aria-hidden="true" />}</span><div className="account-card-heading"><CategoryBadge kind={account.kind} label={t(holdingAccountLabel(account.kind))}/><h2>{account.name}</h2></div><AccountMenu name={account.name}><DropdownMenuItem onSelect={()=>setDraft(account)}>{t('Edit account')}</DropdownMenuItem><DropdownMenuItem onSelect={()=>onAdd('Cash',account.id)}>{t('Add cash balance')}</DropdownMenuItem></AccountMenu></header>
     ...
     <div className="account-card-actions"><Button onClick={()=>onAdd(account.kind,account.id)}><Plus size={16} aria-hidden="true" />{t('Add holding')}</Button></div>
