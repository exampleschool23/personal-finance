...
const fresh = (): Entry => ({ id: crypto.randomUUID(), name: '', kind: 'Cash', currency: 'USD', amount: 0, quantity: 1, cost: 0, rate: 0, date: today(), lent_date: today(), frequency: 'Once', notes: '' });
const sample = (): Entry[] => [
...
export default function FinanceWorkspace() {
    ...
    useEffect(() => {
        ...
        fetch('/api/records?' + params, { signal: controller.signal }).then(async response => {
            const data = await response.json() as { error?: string; records: Entry[]; total: number; page: number; summary?: Entry[] };
            if (!response.ok) throw Error(data.error);
            ...
            if (data.summary) { setSummary(data.summary.map(normalize)); summaryLoaded.current = true; }
    ...
    } setUser(null); setDemo(false); setRows([]); setSummary([]); setRecordTotal(0); setPreferencesData(defaultPreferences); setCurrency('USD'); summaryLoaded.current = false; setError(''); }
