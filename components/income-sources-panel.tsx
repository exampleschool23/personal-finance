...
export function IncomeSourcesPanel({controller,currencies,records,onRecord}:{controller:EarningSourcesController;currencies:string[];records:Entry[];onRecord:(source:EarningSource,bonus?:boolean)=>void}){
 ...
 {controller.loading?<p>{t('Loading records…')}</p>:controller.error?<p role="alert" className="error">{t(controller.error)}</p>:<ul className="tool-list">{controller.sources.map(source=><li key={source.id}><div><strong>{source.name}</strong><p className="muted">{t(source.archived?'Archived':source.mode==='fixed'?'Fixed income':'Variable income')} · {t(source.kind)}{source.mode==='fixed'&&<> · {formatMoney(source.amount??0,source.currency,locale)} · {t(frequencyLabels[source.frequency??'Monthly'])}</>}</p></div><div className="flex flex-wrap gap-2">{!source.archived&&<><Button disabled={busy} onClick={()=>onRecord(source)}>{t('Record income')}</Button><Button variant="outline" disabled={busy} onClick={()=>onRecord(source,true)}>{t('Record bonus')}</Button></>}<Button variant="outline" disabled={busy} onClick={()=>setDraft(source)}>{t('Edit')}</Button><Button variant="ghost" disabled={busy} onClick={()=>void archive(source)}>{t(source.archived?'Restore':'Archive')}</Button></div></li>)}</ul>}
