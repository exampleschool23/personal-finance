...
See [implementation status and remaining work](docs/roadmap-implementation.md). The current schema includes migrations through 071. Apply only unapplied migrations in order; live migration status is not inferred from repository files. See [record reliability and restoration](docs/reliability-and-restore.md) for the new upgrade and restore limits.
