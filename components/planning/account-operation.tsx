...
export function AccountOperation({operation,records,save,onClose}:{operation:Operation;records:Entry[];save:(action:string,data:unknown)=>Promise<void>;onClose:()=>void}){
 ...
 const accounts=records.filter(r=>r.kind==='Cash'&&(!['repayment','mortgage','occurrence'].includes(operation.action)||!target||r.currency===target.currency));
 ...
 {['transfer','repayment','mortgage'].includes(operation.action)&&<label>{t(operation.action==='transfer'?'Transfer fee':target?.kind==='Money lent'?'Interest received':'Interest paid')} {account?.currency}<FormattedNumberInput required={false} value={draft.fee} onValueChange={fee=>setDraft({...draft,fee})}/></label>}
 ...
 <div className="record-form-footer"><Button type="button" variant="outline" disabled={busy} onClick={onClose}>{t('Cancel')}</Button><Button disabled={busy||!draft.account_id}>{t(busy?'Saving…':submitted?'Retry':'Save')}</Button></div>
