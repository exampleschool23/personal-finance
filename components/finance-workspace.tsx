...
import { defaultPreferences, fiatCurrencies, currencyLabel, type Preferences } from '@/lib/currencies';
...
const fresh = (): Entry => ({ id: crypto.randomUUID(), name: '', kind: 'Cash', currency: 'USD', amount: 0, quantity: 1, cost: 0, rate: 0, date: today(), lent_date: today(), frequency: 'Once', notes: '', business_id: null });
const sample = (): Entry[] => [
...
export default function FinanceWorkspace() {
    ...
    useEffect(() => {
        ...
        fetch('/api/records?' + params, { signal: controller.signal }).then(async response => {
            ...
            const normalize = (e: Entry): Entry => ({ ...e, date: e.date ?? '', lent_date: e.lent_date ?? '', amount: Number(e.amount), quantity: Number(e.quantity), cost: Number(e.cost), rate: Number(e.rate) });
    ...
    const current = (demo ? rows : summary).map(r => marketEntry(r, currency, market)).filter((r): r is Entry => r !== null), sum = (types: readonly string[]) => current.filter(r => types.includes(r.kind)).reduce((n, r) => n + value(r), 0), totalAssets = sum(assets), totalDebt = sum(liabilities), monthlyIncome = current.filter(r => income.includes(r.kind)).reduce((n, r) => n + monthly(r), 0), monthlyExpense = current.filter(r => expenses.includes(r.kind)).reduce((n, r) => n + monthly(r), 0);
    ...
    const demoVisible = sortRecords(current.filter(r => section === 'Overview' || (section === 'Assets & investments' ? assets : section === 'Loans & debts' ? liabilities : [...income, ...expenses]).includes(r.kind)));
    ...
    const visible = demo ? demoVisible.slice((page - 1) * 20, page * 20) : tableLoading ? [] : rows.map(r => marketEntry(r, currency, market) ?? r);
    ...
    const addRecord = () => {
        ...
        setError('');
        setRecordKinds(section === 'Assets & investments' ? assets : section === 'Loans & debts' ? liabilities : kinds);
        setEditing({ ...fresh(), currency, kind: section === 'Loans & debts' ? 'Mortgage' : 'Cash' });
    };
    const editRecord = (record: Entry) => {
        setError('');
        setRecordKinds(income.includes(record.kind) ? income : expenses.includes(record.kind) ? expenses : assets.includes(record.kind) ? assets : liabilities);
        setEditing({ ...(rows.find(r => r.id === record.id) || record) });
    };
    const field = (key: keyof Entry, v: string | number) => setEditing(p => p ? { ...p, [key]: v } : p);
    return <SidebarProvider><Sidebar><SidebarHeader className="p-6">{brand}</SidebarHeader><SidebarContent className="px-4 pt-8"><p className="nav-label">{t("WORKSPACE")}</p><SidebarMenu>{sections.map(([name, Icon, path]) => <SidebarMenuItem key={name}><SidebarMenuButton asChild className="nav-item" isActive={section === name}><Link href={path} aria-current={section === name ? 'page' : undefined}><Icon /><span>{t(name)}</span></Link></SidebarMenuButton></SidebarMenuItem>)}</SidebarMenu><div className="sidebar-card"><ShieldCheck size={23}/><strong>{t("Your money, your space.")}</strong><p>{demo ? t("Explore with sample data. Changes last until you leave this workspace.") : t("Your records are private to your account.")}</p></div></SidebarContent><SidebarFooter className="p-5"><div className="user-line"><span className="avatar">{demo ? 'D' : user?.slice(0, 1).toUpperCase()}</span><div><strong>{demo ? t("Demo workspace") : t("Personal account")}</strong><p>{demo ? t("Sample data") : user}</p></div></div><Button variant="ghost" onClick={logout}><LogOut size={16}/>{demo ? t("Exit demo") : t("Sign out")}</Button></SidebarFooter></Sidebar><main className="workspace"><header className="topbar"><div className="flex items-center gap-3"><SidebarTrigger aria-label={t('Toggle Sidebar')}/><span>{t("My workspace")}</span><ChevronRight size={14}/><span className="muted">{t(section)}</span></div>{preferences}</header><div className="content">{section === 'Settings' ? <SettingsPanel key={String(user) + settingsLoading} initial={preferencesData} demo={demo} onSaved={applyPreferences} loading={settingsLoading} loadError={settingsError}/> : <>{demo && <div className="demo-banner"><span>{t("DEMO MODE")}</span> {t("Sample balances · Changes are not saved to an account.")}</div>}<div className="page-heading"><div><p className="eyebrow">{t("FINANCIAL WORKSPACE")}</p><h1>{section === 'Overview' ? t("Your money at a glance.") : t(section)}</h1><p className="muted">{section === 'Overview' ? t("Everything you own, earn, and owe. In one place.") : cashFlowSection ? t("Plan recurring income and expenses, or add a one-time entry.") : t("Manage your records and keep your balances up to date.")}</p></div><div className="entry-actions">{cashFlowSection ? <><Button variant="outline" onClick={() => addCashFlow('Other income')}><Plus size={18}/>{t("Add income")}</Button><Button className="primary" onClick={() => addCashFlow('Other expense')}><Plus size={18}/>{t("Add expense")}</Button></> : <Button className="primary" onClick={addRecord}><Plus size={18}/>{t("Add record")}</Button>}</div></div><div className="currency-bar"><div className="currency-switch" aria-label={t("Display currency")}>{preferencesData.currencies.map(c => <button key={c} aria-pressed={currency === c} className={currency === c ? 'selected' : ''} onClick={() => setCurrency(c)}>{c === 'USD' ? '$ USD' : 'UZS'}</button>)}</div><span>{t('Balances converted to {currency}.', { currency })}</span></div><div className="market-bar"><span>{market?.ratesDate && currency !== 'UZS' ? t('Daily exchange rates · {date}', { date: formatDate(market.ratesDate) }) : market?.fx ? t('1 USD = {rate} UZS · CBU · {date}', { rate: formatNumber(market.fx.rate, locale), date: formatDate(market.fx.date) }) : t(marketLoading ? 'Fetching prices…' : 'Exchange rate unavailable. Only records in the selected currency are included.')}</span><Button variant="outline" size="sm" disabled={marketLoading} onClick={refresh}>{t(marketLoading ? 'Fetching prices…' : 'Refresh prices')}</Button></div>{market?.ratesDate && <p className="fx-attribution"><a href="https://www.exchangerate-api.com" target="_blank" rel="noreferrer">Rates By Exchange Rate API</a> · {formatDate(market.ratesDate)}</p>}{(demo ? rows : summary).some(r => marketEntry(r, currency, market) === null) && <p className="muted">{t('Some currencies could not be converted and are excluded from totals.')}</p>}{marketError && <p role="status" className="muted">{t(marketError)}</p>}{error && <div className="error" role="alert">{t(error)}</div>}{cashFlowSection ? <div className="metrics recurring-metrics"><article><p>{t("Monthly recurring income")}</p><h2 className="positive">{money(monthlyIncome)}</h2><small>{t("Monthly amounts plus yearly amounts ÷ 12")}</small></article><article><p>{t("Monthly recurring expenses")}</p><h2>{money(monthlyExpense)}</h2><small>{t("Monthly amounts plus yearly amounts ÷ 12")}</small></article><article><p>{t("Left after recurring expenses")}</p><h2 className={monthlyIncome - monthlyExpense >= 0 ? 'positive' : 'negative'}>{money(monthlyIncome - monthlyExpense)}</h2><small>{t("One-time entries are excluded")}</small></article></div> : <div className="metrics"><article className="net-worth"><p>{t("NET WORTH")} <Wallet size={18}/></p><h2>{money(totalAssets - totalDebt)}</h2><small>{t("Assets minus outstanding debt")}</small><div className="net-line"/></article><article><p>{t("Total assets")} <Landmark size={18}/></p><h2>{money(totalAssets)}</h2><small>{t('Assets in {currency}: {count}', { currency, count: current.filter(r => assets.includes(r.kind)).reduce((n,r) => n + (r.record_count ?? 1), 0) })}</small></article><article><p>{t("Outstanding debt")} <HandCoins size={18}/></p><h2>{money(totalDebt)}</h2><small>{t("Mortgages, loans & other debts")}</small></article><article><p>{t("Monthly cash flow")} <ArrowUpRight size={18}/></p><h2 className={monthlyIncome - monthlyExpense >= 0 ? 'positive' : 'negative'}>{money(monthlyIncome - monthlyExpense)}</h2><small>{t("Recurring income minus expenses")}</small></article></div>}{section === 'Assets & investments' && <AssetDistribution entries={current} currency={currency}/>}{section === 'Overview' && <div className="insights"><section className="panel"><div className="panel-title"><h2>{t("Asset allocation")}</h2><span>{t("Current balances")}</span></div>{allocation.length ? <><div className="allocation-bar">{allocation.map((a, i) => <div key={a.kind} style={{ width: `${a.total / totalAssets * 100}%`, background: ['#c4f36b', '#739f80', '#8b9fe4', '#e6bd73', '#9b85b5', '#70bcb8'][i] }}/>)}</div><div className="allocation-list">{allocation.map((a, i) => <div key={a.kind}><span><i style={{ background: ['#c4f36b', '#739f80', '#8b9fe4', '#e6bd73', '#9b85b5', '#70bcb8'][i] }}/>{t(a.kind)}</span><strong>{money(a.total)}</strong><span>{Math.round(a.total / totalAssets * 100)}%</span></div>)}</div></> : <div className="empty"><Landmark /><p>{t("Add your first asset to see its allocation.")}</p></div>}</section><section className="panel"><div className="panel-title"><h2>{t("Monthly commitments")}</h2><span>{t("Recurring only")}</span></div><div className="cash-row"><span className="icon-box"><ArrowDownLeft /></span><div><p>{t("Income")}</p><small>{t("Salary, rent received & more")}</small></div><strong className="positive">{money(monthlyIncome)}</strong></div><div className="cash-row"><span className="icon-box outgoing"><ArrowUpRight /></span><div><p>{t("Expenses")}</p><small>{t("Rent, living costs & more")}</small></div><strong>{money(monthlyExpense)}</strong></div><div className="cash-footer"><span>{t("Left after expenses")}</span><strong>{money(monthlyIncome - monthlyExpense)}</strong></div><p className="footnote">{t("Yearly records are divided by 12. One-time records are excluded.")}</p></section></div>}<section className="panel records"><div className="panel-title"><h2>{section === 'Overview' ? t("All records") : t(section)} <span className="count">{formatNumber(totalRecords, locale, 0)}</span></h2><span>{t("Fetched prices where available")}</span></div>{tableLoading ? <div className="empty" role="status">{t("Loading records…")}</div> : visible.length ? <div className="table-scroll"><table><thead><tr><th>{t("Name")}</th><th>{t("Category")}</th><th>{cashFlowSection ? t("Start / record date") : t("Date / due date")}</th>{cashFlowSection && <th>{t("Repeats")}</th>}<th>{cashFlowSection ? t("Amount per occurrence") : t("Value")}</th><th><span className="sr-only">{t("Actions")}</span></th></tr></thead><tbody>{visible.map(r => <tr key={r.id}><td><div className="record-name"><span className="record-icon">{r.kind === 'Crypto' ? <Bitcoin /> : r.kind === 'Property' || r.kind === 'Mortgage' ? <Building2 /> : <Wallet />}</span><div><strong>{r.name}</strong>{r.business_id && <small>{t("Business: {name}", { name: availableBusinesses.find(b => b.id === r.business_id)?.name || t("Business") })}</small>}<small>{['Stock', 'Crypto'].includes(r.kind) ? t('{quantity} units · Gain/loss {amount}', { quantity: formatNumber(r.quantity, locale), amount: money((r.amount - r.cost) * r.quantity, r.currency) }) : r.frequency === 'Once' ? (r.rate ? t('{rate}% annual interest', { rate: formatNumber(r.rate, locale) }) : r.notes || t("One-time record")) : t(r.frequency)}</small>{['Stock', 'Crypto'].includes(r.kind) && <><small>{t('{price} per unit', { price: formatMoney(r.amount, r.currency, locale, true) })}</small><small>{quoteLabel(r)}</small></>}</div></div></td><td><span className="badge">{t(r.kind)}</span></td><td className="muted">{r.kind === 'Money lent' ? <><div>{t("Lent: {date}", { date: formatDate(r.lent_date || '') })}</div><small>{r.date ? t("Due: {date}", { date: formatDate(r.date) }) : t("No due date")}</small></> : formatDate(r.date)}</td>{cashFlowSection && <td><span className="badge">{r.frequency === 'Once' ? t("One time") : r.frequency === 'Monthly' ? t("Every month") : t("Every year")}</span></td>}<td className="amount">{money(value(r), r.currency)}</td><td><div className="row-actions"><Button size="icon" variant="ghost" aria-label={t('Edit {name}', { name: r.name })} onClick={() => editRecord(r)}><Pencil size={15}/></Button><Button size="icon" variant="ghost" aria-label={t('Delete {name}', { name: r.name })} onClick={() => setDeleting(r)}><Trash2 size={15}/></Button></div></td></tr>)}</tbody></table></div> : <div className="empty"><Wallet /><h3>{t("A fresh start.")}</h3><p>{t('Add a record in {currency} to start building your overview.', { currency })}</p><Button variant="outline" onClick={addRecord}><Plus />{cashFlowSection ? t("Add your first expense") : t("Add your first record")}</Button></div>}<nav className="records-pagination" aria-label={t('Record pages')}><span>{t('Page {page} of {pages} · {count} records', { page: formatNumber(page, locale, 0), pages: formatNumber(pageCount, locale, 0), count: formatNumber(totalRecords, locale, 0) })}</span><div><Button variant="outline" disabled={tableLoading || busy || page <= 1} onClick={() => setPageState({ key: paginationKey, page: page - 1 })}>{t('Previous')}</Button><Button variant="outline" disabled={tableLoading || busy || page >= pageCount} onClick={() => setPageState({ key: paginationKey, page: page + 1 })}>{t('Next')}</Button></div></nav></section><p className="bottom-note"><ShieldCheck size={14}/>{demo ? t("Sample data for exploring the app.") : t("Private records · Only visible to your account.")}</p></>}</div></main>
 <Dialog open={!!editing} onOpenChange={open => { if (!open && !busy)
        setEditing(null); }}><DialogContent className="record-dialog" showCloseButton={false}><DialogClose className="absolute top-4 right-4" aria-label={t('Close')} disabled={busy}><X size={18}/></DialogClose><DialogTitle>{rows.some(r => r.id === editing?.id) ? t("Edit record") : editingCashFlow ? (income.includes(editing!.kind) ? t("Add income") : t("Add expense")) : editing?.kind === 'Money lent' ? t("Add money lent") : t("Add a record")}</DialogTitle><DialogDescription>{editing?.kind === 'Money lent' ? t("Keep track of who owes you, how much, and when.") : editingCashFlow ? t("Enter an amount and choose how often it repeats.") : (recordKinds === assets || recordKinds === liabilities ? t("Keep a current balance for this record.") : t("Keep a current balance or record income and expenses."))}</DialogDescription>{editing && <form className="record-form" onSubmit={save}>{editing.kind === 'Crypto' ? <label>{t('Coin')}<select value={coins.find(c => c[0] === instrumentFor(editing)?.symbol) ? coinName(coins.find(c => c[0] === instrumentFor(editing)?.symbol)!) : ''} required onChange={e => setEditing({ ...editing, name: e.target.value, amount: 0 })}><option value="">{t('Select a coin')}</option>{coins.map(c => <option key={c[0]} value={coinName(c)}>{coinName(c)}</option>)}</select></label> : editing.kind === 'Stock' ? <label>{t('Stock ticker (USD-listed)')}<Input value={editing.name} onChange={e => setEditing({ ...editing, name: e.target.value.toUpperCase().trim(), amount: 0 })} placeholder="AAPL" pattern="[A-Z][A-Z0-9.\-]{0,14}" maxLength={15} required /></label> : <RecordNameInput label={editing.kind === 'Money lent' ? t("Borrower name") : undefined} entry={editing} original={rows.find(r => r.id === editing.id)} rows={demo ? rows : summary} onChange={name => field('name', name)} placeholder={editingCashFlow ? (income.includes(editing.kind) ? t("e.g. Monthly salary") : t("e.g. Rent or internet subscription")) : editing.kind === 'Money lent' ? t("e.g. Full name") : editing.kind === 'Business' ? t("e.g. Solar panels, café, or game club") : t("e.g. Savings account")}/>}<div className="form-grid"><label>{t("Category")}<select value={editing.kind} onChange={e => setEditing({ ...editing, kind: e.target.value as Entry['kind'], name: '', amount: 0, business_id: [...income, ...expenses].includes(e.target.value) ? editing.business_id : null, date: e.target.value === 'Money lent' ? '' : editing.date || today(), lent_date: editing.lent_date || today() })}>{recordKinds.map(k => <option key={k} value={k}>{t(k)}</option>)}</select></label><label>{t("Currency")}<select value={editing.currency} onChange={e => field('currency', e.target.value)}>{[...new Set([...preferencesData.currencies, editing.currency, ...fiatCurrencies.map(c => c.code)])].map(code => <option key={code} value={code}>{currencyLabel(code, locale)}</option>)}</select></label></div>{editingCashFlow && <label>{t('Linked business (optional)')}<select value={editing.business_id || ''} onChange={event => setEditing({ ...editing, business_id: event.target.value || null })}><option value="">{t('No linked business')}</option>{availableBusinesses.map(business => <option key={business.id} value={business.id}>{business.name}</option>)}</select></label>}{editing.kind === 'Business' && <p className="muted">{t('Enter the value of your ownership. Record revenue and costs separately in Income & expenses, linked to this business.')}</p>}<label className="record-amount">{['Stock', 'Crypto'].includes(editing.kind) ? t("Current price per unit") : editingCashFlow ? t("Amount per occurrence") : editing.kind === 'Money lent' ? t("Amount still owed") : editing.kind === 'Business' ? t("Current business value") : t("Amount / outstanding balance")}<FormattedNumberInput key={editing.id + ":amount"} value={editing.amount} max={1e15} onValueChange={value => field('amount', value)}/></label>{['Stock', 'Crypto'].includes(editing.kind) && <><Button type="button" variant="outline" disabled={fetchingPrice || !instrumentFor(editing)} onClick={fetchPrice}>{t(fetchingPrice ? 'Fetching prices…' : 'Fetch current price')}</Button>{priceMessage && <p role="status" className="muted">{t(priceMessage)}</p>}</>}{['Stock', 'Crypto'].includes(editing.kind) && <div className="form-grid"><label>{t("Quantity")}<FormattedNumberInput key={editing.id + ":quantity"} value={editing.quantity} max={1e12} onValueChange={value => field('quantity', value)}/></label><label>{t("Purchase price per unit")}<FormattedNumberInput key={editing.id + ":cost"} value={editing.cost} required={false} max={1e15} onValueChange={value => field('cost', value)}/></label></div>}{['Deposit', 'Money lent', ...liabilities].includes(editing.kind) && <label>{t("Annual interest rate (%)")}<FormattedNumberInput key={editing.id + ":rate"} value={editing.rate} required={false} max={1000} onValueChange={value => field('rate', value)}/></label>}<div className="form-grid">{editing.kind === 'Money lent' && <label>{t("Date lent")}<DatePicker value={editing.lent_date || ''} onChange={value => field('lent_date', value)}/></label>}<label>{editing.kind === 'Money lent' ? t("Due date (optional)") : ['Deposit', ...liabilities].includes(editing.kind) ? t("Due / maturity date") : editingCashFlow && editing.frequency !== 'Once' ? t("Start date") : t("Record date")}<DatePicker value={editing.date} required={editing.kind !== 'Money lent'} min={editing.kind === 'Money lent' ? editing.lent_date || undefined : undefined} onChange={value => field('date', value)}/></label>{editingCashFlow && <label>{t("Repeats")}<select value={editing.frequency} onChange={e => field('frequency', e.target.value)}><option value="Once">{t("One time")}</option><option value="Monthly">{t("Every month")}</option><option value="Yearly">{t("Every year")}</option></select></label>}</div>{editingCashFlow && editing.frequency !== 'Once' && <p className="recurrence-help">{t('{amount} {frequency} from {date}. This is a recurring plan; it does not automatically create transactions or change account balances.', { amount: money(editing.amount, editing.currency), frequency: t(editing.frequency === 'Monthly' ? 'every month' : 'every year'), date: formatDate(editing.date) })}</p>}<label>{t("Notes (optional)")}<textarea value={editing.notes} maxLength={2000} rows={2} placeholder={t("Account, lender, borrower, or other details")} onChange={e => field('notes', e.target.value)}/></label>{error && <p role="alert" className="error">{t(error)}</p>}<div className="record-form-footer"><Button type="button" variant="outline" disabled={busy} onClick={() => setEditing(null)}>{t("Cancel")}</Button><Button className="primary" disabled={busy}>{busy ? t("Saving…") : demo ? t("Save in demo") : t("Save record")}</Button></div></form>}</DialogContent></Dialog>
