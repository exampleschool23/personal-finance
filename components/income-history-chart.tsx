...
export function IncomeHistoryChart({records,events,incomeRecords,currency,rates,today}:Props){
 ...
 return <section className="panel income-history-chart">
  ...
  {(result.missing>0||result.estimateMissing>0)&&<p role="status" className="comparison-note">{t('Some income cannot be converted. Displayed chart amounts are incomplete.')}</p>}
  ...
  <p className="footnote">{t('Solid bars show recorded one-time income and Tracker receipts. The outlined bar is this month’s estimate, including recurring salary and asset income; it is not added to received income. Older months show recorded payments only.')}</p>
