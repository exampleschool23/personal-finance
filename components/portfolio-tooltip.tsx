...
type TooltipPoint={date:string;net?:number|null;actual?:number|null};
type Props={valueKey?:'net'|'actual';showBalanceDifference?:boolean;valueLabel?:string;active?:boolean;payload?:readonly {payload?:TooltipPoint}[];details:Map<string,PortfolioDetails>;currency:string;onOpenActivity?:(activity:PortfolioChange)=>void};
export function PortfolioTooltip({active,payload,details,currency,onOpenActivity,valueLabel='NET WORTH',valueKey='net',showBalanceDifference=true}:Props){
 ...
 return <div className="portfolio-tooltip" onMouseMove={event=>event.stopPropagation()} onTouchMove={event=>event.stopPropagation()} onWheel={event=>event.stopPropagation()}>
  <header><span>{formatDate(point.date,locale)}</span><small>{t(valueLabel)}</small><strong>{typeof value==='number'?money(value):'—'}</strong></header>
