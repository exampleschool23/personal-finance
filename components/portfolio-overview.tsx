...
export function PortfolioOverview({ entries, currency, market, demo, revision }: { entries: Entry[]; currency: string; market: MarketData | null; demo: boolean; revision: number }) {
 ...
 const points = [...tracked.points.filter(point => point.date < today), { date: today, assets: assetTotal, debt, net: assetTotal - debt }];
 ...
 return <>
  <section className="panel portfolio-trend">
   ...
   {loading ? <LoadingPlaceholder label={t('Loading history…')}/> : error ? <p role="alert" className="error">{t('Could not load portfolio history.')} <Button variant="outline" onClick={() => { setError(false); setHistory(null); setRetry(n => n + 1); }}>{t('Retry')}</Button></p> : <>
    {visible.length > 1 ? <div className="portfolio-chart" aria-label={t('Portfolio over time')}><ResponsiveContainer width="100%" height={310}><LineChart data={visible.map(point => ({ ...point, timestamp: Date.parse(point.date + 'T00:00:00Z') }))} accessibilityLayer margin={{ top: 15, right: 15, left: 5, bottom: 10 }}>
     <CartesianGrid stroke="var(--border)" strokeDasharray="3 5" vertical={false}/>
     <XAxis dataKey="timestamp" type="number" scale="time" domain={['dataMin', 'dataMax']} tickFormatter={date => formatDate(historyChartDate(Number(date)), locale)} minTickGap={65} tick={{ fontSize: 11 }}/>
     <YAxis width={110} tickFormatter={money} tick={{ fontSize: 11 }}/>
     ...
     <Legend/>
     <Line type="stepAfter" dataKey="assets" name={t('Total assets')} stroke="#0d9488" strokeWidth={2} dot={false}/>
     <Line type="stepAfter" dataKey="debt" name={t('Outstanding debt')} stroke="#e18445" strokeWidth={2} strokeDasharray="5 4" dot={false}/>
     <Line type="stepAfter" dataKey="net" name={t('NET WORTH')} stroke="var(--primary)" strokeWidth={3} dot={visible.length < 15} activeDot={{ r: 5 }}/>
    </LineChart></ResponsiveContainer></div> : <div className="portfolio-history-empty"><strong>{t('Your first snapshot is ready')}</strong><p>{t('Add dated balance updates in Tracker to see your portfolio evolve. A trend needs at least two dates with balances for every holding.')}</p><span>{formatDate(today, locale)}</span></div>}
    ...
    <p className="footnote">{t('Uses today’s exchange rates and carries recorded balances forward. Today includes available market quotes. Changes include money added or removed, not just investment returns. Removed holdings are excluded.')}</p>
   </>}
  </section>
