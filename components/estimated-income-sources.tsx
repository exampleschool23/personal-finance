...
import { income, monthly, duplicatesAssetEstimate, type Entry } from '@/lib/finance';
...
export function EstimatedIncomeSources({ entries, currency, month }: { entries: Entry[]; currency: string; month: string }) {
 ...
 return <section className="panel income-estimates">
  ...
  {!!excluded.length && <details className="excluded-income" open={excluded.some(entry=>entry.kind==='Salary')}><summary>{t('Income not included this month')}<ChevronDown size={16}/></summary><ul>{excluded.map(entry => <li key={entry.id}><div><strong>{entry.name || t(entry.kind)}</strong><CategoryBadge kind={entry.kind} label={t(entry.kind)}/></div><span>{t(entry.frequency === 'Once' ? 'One-time entry. Choose Every month to include a recurring salary.' : duplicatesAssetEstimate(entry,businessIds,propertyIds) ? (entry.kind==='Rent income'?'Already included in the property estimate.':'Already included in the business estimate.') : 'Outside its start and end dates for this month.')}</span></li>)}</ul><a href="#workspace-records">{t('Review income records')}</a></details>}
 </section>;
}
