...
function WorkspaceContent() {
    ...
    const planning = usePlanning(user, demo, rows, reload, refreshRecords, demoHoldingAccounts);
    ...
    useEffect(() => {
        ...
        const params = new URLSearchParams({ page: String(page), section: sectionKey, summary: summaryCache.current.loaded && summaryCache.current.revision === reload ? '0' : '1' });
    ...
    async function save(e: React.FormEvent) { e.preventDefault(); if (!editing)
        ...
        const savedRecord={...editing,...(liabilities.includes(editing.kind)&&!rows.some(row=>row.id===editing.id)?{opened_on:editing.opened_on??today()}:{})};
        ...
        if (demo) setRows(prev => [savedRecord, ...prev.filter(r => r.id !== savedRecord.id)]);
        else refreshRecords();
        setEditing(null);
    }
    catch (e) {
    ...
    } setLoadedKey(''); setSettingsLoading(true); setUser(null); setDemo(false); setDemoHoldingAccounts([]); setDeletedItems([]); setRows([]); setSummary([]); setBusinesses([]); setRecordTotal(0); setPreferencesData(defaultPreferences); setCurrency('USD'); setSummaryLoaded(false); summaryCache.current = {loaded:false,revision:-1}; setError(''); }
