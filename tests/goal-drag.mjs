...
test('reordered cards animate from previous positions and honor reduced motion',()=>{
 ...
 try{
  ...
  function render(order){cursor=0;effects.length=0;const ref=useGoalLayoutAnimation(order);ref.current={querySelectorAll:()=>cards};effects[0]();}
  render('a,b');assert.equal(calls.length,0);
  positions=[120,0];render('b,a');assert.equal(calls.length,2);assert.equal(calls[0].frames[0].transform,'translate(-120px, 0px)');assert.equal(calls[1].frames[0].transform,'translate(120px, 0px)');
  reduced=true;positions=[0,120];render('a,b');assert.equal(calls.length,2);assert.equal(cancelled,2);
