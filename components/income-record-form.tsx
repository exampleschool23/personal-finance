...
import { selectEarningSource } from '@/lib/earning-sources';
...
export function IncomeRecordForm({editing,setEditing,busy,save,rows,currencies,planning,error,demo,earningSources,onNavigateToSources}:RecordDialogProps & {onNavigateToSources?:()=>void}){
 ...
 return <form className="record-form" onSubmit={save}>
  {simple&&earningSources&&<div><label>{t('Income source')}<NativeSelect value={editing.earning_source_id??(retained?'saved':editing.kind==='Other income'?'':'choose')} disabled={busy} onChange={event=>{
   const item=earningSources.sources.find(source=>source.id===event.target.value);
   setSalaryPlan(false);
   if(item)update(selectEarningSource(editing,item));
   else if(event.target.value==='saved'&&original)update({kind:original.kind,name:original.name,business_id:original.business_id,income_source_id:original.income_source_id,income_due_on:original.income_due_on,earning_source_id:null,earning_due_on:null,payment_type:'regular'});
   else setEditing({...changeIncomeKind(editing,'Other income'),earning_source_id:null,earning_due_on:null,payment_type:'regular'});
  }}>
   {editing.kind!=='Other income'&&!retained&&!editing.earning_source_id&&<option value="choose" disabled>{t('Choose an income source')}</option>}
   <option value="">{t('Other income')}</option>
   {original&&!original.earning_source_id&&original.kind!=='Other income'&&<option value="saved">{original.name}</option>}
   {editing.earning_source_id&&!reusable&&<option value={editing.earning_source_id} disabled>{editing.name}</option>}
   {earningSources.sources.filter(item=>!item.archived||item.id===original?.earning_source_id).map(item=><option key={item.id} value={item.id} disabled={earningSources.loading||!!earningSources.error}>{item.name}</option>)}
  </NativeSelect></label><Button asChild variant="outline" className="mt-3 min-h-11 w-full" disabled={busy}><Link href="/income-expenses#income-sources" aria-disabled={busy} onNavigate={event=>{if(busy){event.preventDefault();return;}(onNavigateToSources??(()=>setEditing(null)))();}}><Settings2 aria-hidden="true"/>{t('Manage income sources')}<ArrowRight aria-hidden="true"/></Link></Button>
  ...
  {reusable&&editing.payment_type!=='bonus'&&reusable.mode==='fixed'&&<label>{t('Scheduled payment date')}<DatePicker value={editing.earning_due_on??editing.date} min={reusable.start_date??undefined} max={reusable.end_date??undefined} onChange={earning_due_on=>update({earning_due_on})}/></label>}
  ...
  {editing.kind==='Salary'&&source&&<label>{t('Salary due date')}<DatePicker value={editing.income_due_on??editing.date} min={source.date} max={source.end_date??undefined} onChange={income_due_on=>update({income_due_on})}/><small className="muted">{t('The scheduled payment this receipt settles. Record date is when you received the money.')}</small></label>}
  <AmountCurrencyFields amount={editing.amount} currency={editing.currency} currencies={currencies} savedCurrency={original?.currency} disabled={busy} label={t(salaryPlan?'Amount per occurrence':'Amount')} onAmountChange={amount=>update({amount})} onCurrencyChange={currency=>update({currency,account_exchange_rate:null,account_rate_date:null,account_currency:null})}/>
