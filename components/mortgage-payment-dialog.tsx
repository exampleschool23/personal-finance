...
export type MortgagePayment = { id: string; mortgage_id: string; principal: number; interest: number; date: string; notes: string };
export function MortgagePaymentDialog({ mortgage, onClose, onSave }: { mortgage: Entry; onClose: () => void; onSave: (payment: MortgagePayment) => Promise<void> }) {
 const { t, locale } = useLanguage();
 const [payment, setPayment] = useState<MortgagePayment>(() => ({ id: crypto.randomUUID(), mortgage_id: mortgage.id, principal: 0, interest: 0, date: new Date().toISOString().slice(0,10), notes: '' }));
 ...
 return <Dialog open onOpenChange={open => { if (!open && !busy) onClose(); }}><DialogContent className="record-dialog" showCloseButton={!busy}>
  ...
  <form className="record-form" onSubmit={async event => {
   ...
   <fieldset disabled={busy || submitted} className="grid gap-5 border-0 p-0 m-0 min-w-0">
    ...
    <label>{t('Payment date')}<DatePicker value={payment.date} onChange={date => setPayment({ ...payment, date })}/></label>
   ...
   <p className="muted">{t('Saved once in Income & expenses. Cash account balances are not changed automatically. Saved payments cannot be edited or deleted.')}</p>
