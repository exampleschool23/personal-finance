...
export function movementSources(kind: MovementKind, records: Entry[]) {
 return records.filter(record => kind === 'transfer' ? isBalanceAccount(record) : kind === 'interest' ? record.kind === 'Deposit' : kind === 'sell' ? isHolding(record) : record.kind === 'Cash' || record.kind === 'Crypto');
}
export function movementTargets(kind: MovementKind, source: Entry | undefined, records: Entry[]) {
 return records.filter(record => record.id !== source?.id && (kind === 'transfer' ? isBalanceAccount(record) : kind === 'buy' ? isHolding(record) : record.kind === 'Cash' || record.kind === 'Crypto'));
}
