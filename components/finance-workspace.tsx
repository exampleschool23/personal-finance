...
import { DataTools } from '@/components/planning/data-tools';
...
import { ArrowUpRight, ArrowDownLeft, GitCompareArrows, ChartNoAxesCombined, Wallet, ShieldCheck, LayoutDashboard, Landmark, HandCoins, Plus, LogOut, Pencil, Trash2, ChevronRight, Bitcoin, Building2, Settings } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { LanguageProvider, LanguageSelector, useLanguage } from '@/components/language-provider';
import { ThemeToggle } from '@/components/theme-provider';
import { formatMoney, formatNumber, formatDate as sharedFormatDate, formatDateTime, formatMonthYear } from '@/lib/format';
import { ExpensePlans } from '@/components/expense-plans';
import { shiftCalendarMonth } from '@/lib/date-picker-calendar';
...
import { compareRecordDates, matchesRecordDate } from '@/lib/record-dates';
...
function WorkspaceContent() {
    ...
    const snapshots = usePortfolioSnapshots(demo ? null : user, market, summaryLoaded.current && !marketLoading, reload);
    ...
    const filtersActive = sectionKey !== 'assets' && (filters.query.trim() !== '' || filters.category !== 'all' || !!filters.from || !!filters.to || filters.order !== 'newest');
    const paginationKey = sectionKey + ':' + currencyFilter + ':' + JSON.stringify(filters);
    ...
    const planning = usePlanning(user, demo, rows, reload, refreshRecords);
    ...
    useEffect(() => {
        ...
        fetch('/api/records?' + params, { signal: controller.signal }).then(async response => {
            ...
            if (data.summary) { setSummary(data.summary.map(normalizeEntry)); setBusinesses(data.businesses || []); summaryLoaded.current = true; }
        ...
        return () => controller.abort();
    }, [user, demo, page, section, sectionKey, currencyFilter, reload, paginationKey, requestKey]);
    ...
    } setForecastMonth(expensePlanMonth()); setLoadedKey(''); setSettingsLoading(true); setUser(null); setDemo(false); setDeletedItems([]); setRows([]); setSummary([]); setBusinesses([]); setRecordTotal(0); setPreferencesData(defaultPreferences); setCurrency('USD'); summaryLoaded.current = false; setError(''); }
    ...
    const filteredRecords = (demo ? rows : planning.data.records).filter(r => (sectionKey === 'all' || (sectionKey === 'debts' ? lendingRecordKinds : [...income,...expenses]).includes(r.kind)) && (!currencyFilter || r.currency === currencyFilter) && (!filters.query.trim() || `${r.name} ${r.notes}`.toLocaleLowerCase(locale).includes(filters.query.trim().toLocaleLowerCase(locale))) && (filters.category === 'all' || r.kind === filters.category || r.custom_category_id === filters.category) && matchesRecordDate(r.date, filters.from, filters.to)).sort((a,b)=>filters.order === 'name' ? a.name.localeCompare(b.name,locale) : compareRecordDates(a.date,b.date,filters.order) || a.id.localeCompare(b.id));
    const totalRecords = filtersActive ? filteredRecords.length : demo ? demoVisible.length : recordTotal;
    const pageCount = Math.max(1, Math.ceil(totalRecords / 10));
    const tablePage = Math.min(page,pageCount);
    const tableLoading = !demo && (filtersActive ? planning.loading : recordsLoading || loadedKey !== requestKey);
    const workspaceLoading = !demo && (settingsLoading || (!summaryLoaded.current && tableLoading) || expensePlans.loading || (!market && marketLoading));
    const visible = filtersActive ? filteredRecords.slice((tablePage-1)*10,tablePage*10).map(r=>marketEntry(r,currency,market)??r) : demo ? demoVisible.slice((page - 1) * 10, page * 10) : tableLoading ? [] : sectionKey === 'assets' ? sortAssetsByWorth(rows, r => marketEntry(r, currency, market)).slice((page - 1) * 10, page * 10).map(r => marketEntry(r, currency, market) ?? r) : rows.map(r => marketEntry(r, currency, market) ?? r);
    ...
    return <SidebarProvider><Sidebar><SidebarHeader className="p-6">{brand}</SidebarHeader><SidebarContent className="px-4 pt-8"><p className="nav-label">{t("WORKSPACE")}</p><SidebarMenu>{sections.map(([name, Icon, path]) => <SidebarMenuItem key={name}><SidebarMenuButton asChild className="nav-item" isActive={section === name}><Link href={path} aria-current={section === name ? 'page' : undefined}><Icon /><span>{t(name)}</span>{name === 'Upcoming payments' && overdueCount > 0 && <span className="count">{formatNumber(overdueCount, locale, 0)}</span>}</Link></SidebarMenuButton></SidebarMenuItem>)}</SidebarMenu><div className="sidebar-card"><ShieldCheck size={23}/><strong>{t("Your money, your space.")}</strong><p>{demo ? t("Explore with sample data. Changes last until you leave this workspace.") : t("Your records are private to your account.")}</p></div></SidebarContent><SidebarFooter className="p-5"><div className="user-line"><span className="avatar">{demo ? 'D' : user?.slice(0, 1).toUpperCase()}</span><div><strong>{demo ? t("Demo workspace") : t("Personal account")}</strong><p>{demo ? t("Sample data") : user}</p></div></div><Button variant="ghost" onClick={logout}><LogOut size={16}/>{demo ? t("Exit demo") : t("Sign out")}</Button></SidebarFooter></Sidebar><main className="workspace"><header className="topbar"><div className="flex items-center gap-3"><SidebarTrigger aria-label={t('Toggle Sidebar')}/><span>{t("My workspace")}</span><ChevronRight size={14}/><span className="muted">{t(section)}</span></div><div className="topbar-actions"><Button variant="outline" size="sm" onClick={()=>{setRecordKinds(expenses);setEditing({...fresh(),currency,kind:'Other expense',frequency:'Once'});}}>{t('Quick expense')}</Button>{preferences}</div></header><div data-page={section} className={section === 'Overview' ? 'content overview-content' : section === 'Assets & investments' ? 'content assets-content' : 'content'}>{['Accounts','Upcoming payments','Savings goals','Data & categories'].includes(section) ? <>{planning.loading && !(section === 'Savings goals' && (planning.data.goals.length || planning.data.records.length)) ? <LoadingPlaceholder label={t('Loading records…')}/> : planning.error ? <div className="error" role="alert">{t(planning.error)} <Button onClick={refreshRecords}>{t('Retry')}</Button></div> : section === 'Accounts' ? <AccountsPage data={planning.data} save={planning.save} onAdd={()=>{setRecordKinds(['Cash']);setEditing({...fresh(),currency,kind:'Cash'});}} onEdit={editRecord}/> : section === 'Upcoming payments' ? <UpcomingPage data={planning.data} save={planning.save}/> : section === 'Savings goals' ? <GoalsPage data={planning.data} save={planning.save} currencies={preferencesData.currencies} market={market} plans={expensePlans.plans} plansReady={!expensePlans.loading && !expensePlans.error} snapshots={snapshots.snapshots} historyError={snapshots.error}/> : <DataTools data={planning.data} demo={demo} save={planning.save} onSaved={refreshRecords}/>}</> : section === 'Recently deleted' ? <RecentlyDeleted demo={demo} demoItems={deletedItems} onRestore={restoreDemoItem} onSaved={refreshRecords}/> : section === 'Settings' ? <SettingsPanel key={String(user) + settingsLoading} initial={preferencesData} demo={demo} onSaved={applyPreferences} loading={!demo && settingsLoading} loadError={settingsError}/> : section === 'Benchmarks' ? <BenchmarksPage currency={currency} currencies={preferencesData.currencies} onCurrencyChange={setCurrency} market={market} marketLoading={marketLoading} onRefresh={refresh} demo={demo} revision={reload}/> : <>{demo && <div className="demo-banner"><span>{t("DEMO MODE")}</span> {t("Sample balances · Changes are not saved to an account.")}</div>}<div className="page-heading"><div><h1>{section === 'Overview' ? t("Your money at a glance.") : t(section)}</h1><p className="muted">{section === 'Overview' ? t("Everything you own, earn, and owe. In one place.") : cashFlowSection ? t("Plan recurring income and expenses, or add a one-time entry.") : t("Manage your records and keep your balances up to date.")}</p></div>{section !== 'Overview' && <div className="entry-actions">{cashFlowSection ? <><Button variant="outline" onClick={() => addCashFlow('Other income')}><Plus size={18}/>{t("Add income")}</Button><Button className="primary" onClick={() => addCashFlow('Other expense')}><Plus size={18}/>{t("Add expense")}</Button></> : <Button className="primary" onClick={addRecord}><Plus size={18}/>{t("Add record")}</Button>}</div>}</div><div className="workspace-controls"><div className="currency-bar"><div className="currency-switch" aria-label={t("Display currency")}>{preferencesData.currencies.map(c => <button key={c} aria-pressed={currency === c} className={currency === c ? 'selected' : ''} onClick={() => setCurrency(c)}>{c === 'USD' ? '$ USD' : c}</button>)}</div><span>{t('Balances converted to {currency}.', { currency })}</span></div><div className="market-bar"><span>{market?.ratesDate && currency !== 'UZS' ? t('Daily exchange rates · {date}', { date: formatDate(market.ratesDate) }) : market?.fx ? t('1 USD = {rate} UZS · CBU · {date}', { rate: formatNumber(market.fx.rate, locale), date: formatDate(market.fx.date) }) : t(marketLoading ? 'Fetching prices…' : 'Exchange rate unavailable. Only records in the selected currency are included.')}</span><Button variant="outline" size="sm" disabled={marketLoading} onClick={refresh}>{t(marketLoading ? 'Fetching prices…' : 'Refresh prices')}</Button></div>{market?.ratesDate && <p className="fx-attribution"><a href="https://www.exchangerate-api.com" target="_blank" rel="noreferrer">Rates By Exchange Rate API</a> · {formatDate(market.ratesDate)}</p>}</div>{(demo ? rows : summary).some(r => marketEntry(r, currency, market) === null) && <p className="muted">{t('Some currencies could not be converted and are excluded from totals.')}</p>}{marketError && <p role="status" className="muted">{t(marketError)}</p>}{!cashFlowSection && expensePlans.error && <p role="alert" className="error">{t(expensePlans.error)} <Button variant="outline" onClick={refreshRecords}>{t('Retry')}</Button></p>}{convertedPlanExpenses.some(amount => amount === null) && <p className="muted">{t('Some expense plans could not be converted and are excluded from the forecast.')}</p>}{error && <div className="error" role="alert">{t(error)}</div>}<div className="forecast-month-control"><div><strong>{t('Forecast month')}</strong><p className="muted">{t('Recurring records and monthly plans are included only during their active months. Other estimates use your current records.')}</p></div><div className="forecast-month-actions"><Button variant="outline" size="icon" aria-label={t('Previous month')} onClick={() => setForecastMonth(month => shiftCalendarMonth(month, -1))}>‹</Button><span aria-live="polite">{formatMonthYear(forecastMonth, locale)}</span><Button variant="outline" size="icon" aria-label={t('Next month')} onClick={() => setForecastMonth(month => shiftCalendarMonth(month, 1))}>›</Button><Button variant="outline" disabled={forecastMonth === expensePlanMonth()} onClick={() => setForecastMonth(expensePlanMonth())}>{t('This month')}</Button></div></div>{workspaceLoading ? <WorkspaceSkeleton label={t("Loading your workspace…")} section={section}/> : section === 'Assets & investments' ? <AssetDashboard records={rows} currency={currency} market={market} netWorth={totalAssets-totalDebt} debt={totalDebt} forecast={forecast} forecastReady={forecastReady} loading={tableLoading} demo={demo} onAdd={addRecord} onEdit={editRecord} onTrack={setTracking} onDelete={record => {setError('');setDeleting(record);}} quoteLabel={quoteLabel}/> : <>{cashFlowSection ? <div className="metrics recurring-metrics"><article><p>{t("Estimated monthly income")} <ArrowDownLeft size={18}/></p><h2 className="positive">{money(forecast.plannedIncome)}</h2><small>{t("Includes asset estimates; linked business income counted once.")}</small></article><article><p>{t("Estimated monthly expenses")} <ArrowUpRight size={18}/></p><h2>{forecastReady ? money(forecast.monthlyExpenses + forecast.mortgagePayments) : '—'}</h2><small>{t("Recurring expenses, monthly plans and estimated mortgage payments")}</small></article><article><p>{t("Estimated monthly cash flow")} <ChartNoAxesCombined size={18}/></p><h2 className={forecast.forecast >= 0 ? 'positive' : 'negative'}>{forecastReady ? money(forecast.forecast) : '—'}</h2><small>{t("One-time entries are excluded")}</small></article></div> : section === 'Loans & debts' ? <DebtSummary entries={current} currency={currency}/> : <div className="metrics"><article className="net-worth"><p>{t("NET WORTH")} <Wallet size={18}/></p><h2>{money(totalAssets - totalDebt)}</h2><small>{t("Assets minus outstanding debt")}</small><div className="net-line"/></article><article><p>{t("Total assets")} <Landmark size={18}/></p><h2>{money(totalAssets)}</h2><small>{t('Assets in {currency}: {count}', { currency, count: current.filter(r => assets.includes(r.kind)).reduce((n,r) => n + (r.record_count ?? 1), 0) })}</small></article><article><p>{t("Outstanding debt")} <HandCoins size={18}/></p><h2>{money(totalDebt)}</h2><small>{t("Mortgages, loans & other debts")}</small></article><article><p>{t("Estimated monthly cash flow")} <ArrowUpRight size={18}/></p><h2 className={forecast.forecast >= 0 ? 'positive' : 'negative'}>{forecastReady ? money(forecast.forecast) : '—'}</h2><small>{t("Income estimates minus expenses and mortgage payments")}</small></article></div>}{section === 'Overview' && <PortfolioOverview snapshots={snapshots.snapshots} snapshotError={snapshots.error} onSnapshotRetry={snapshots.retry} key={demo ? 'demo' : user} entries={current} currency={currency} market={market} demo={demo} revision={reload}/>}{(forecast.plannedIncome > 0 || forecast.monthlyExpenses > 0 || forecast.mortgagePayments > 0) && (section === 'Overview' || cashFlowSection || section === 'Loans & debts') && <section className="panel forecast-panel"><div><h2>{t('Estimated monthly cash flow')}</h2><p className="muted">{t('Asset income estimates plus other recurring income, minus recurring expenses and estimated mortgage payments. Linked business income is counted once.')}</p></div><div><strong>{forecastReady ? money(forecast.forecast) : '—'}</strong><small>{t('Estimated asset income: {amount}', { amount: money(forecast.estimatedIncome) })}</small><small>{t('Other recurring income: {amount}', { amount: money(forecast.otherIncome) })}</small><small>{t('Recurring and planned expenses: {amount}', { amount: forecastReady ? money(forecast.monthlyExpenses) : '—' })}</small><small>{t('Estimated mortgage payments: {amount}', { amount: money(forecast.mortgagePayments) })}</small><small>{t('One-time entries are excluded')}</small></div></section>}{section === 'Overview' && <div className="insights"><section className="panel"><div className="panel-title"><h2>{t("Asset allocation")}</h2><span>{t("Current balances")}</span></div>{allocation.length ? <><div className="allocation-bar">{allocation.map((a, i) => <div key={a.kind} style={{ width: `${a.total / totalAssets * 100}%`, background: categoryColor(a.kind) }}/>)}</div><div className="allocation-list">{allocation.map((a, i) => <div key={a.kind}><span><i style={{ background: categoryColor(a.kind) }}/>{t(a.kind)}</span><strong>{money(a.total)}</strong><span>{formatNumber(a.total / totalAssets * 100, locale, 1)}%</span></div>)}</div></> : <div className="empty"><Landmark /><p>{t("Add your first asset to see its allocation.")}</p></div>}</section><section className="panel"><div className="panel-title"><h2>{t("Monthly commitments")}</h2><span>{t("Recurring and planned")}</span></div><div className="cash-row"><span className="icon-box"><ArrowDownLeft /></span><div><p>{t("Income")}</p><small>{t("Includes asset estimates; linked business income counted once.")}</small></div><strong className="positive">{money(forecast.plannedIncome)}</strong></div><div className="cash-row"><span className="icon-box outgoing"><ArrowUpRight /></span><div><p>{t("Expenses")}</p><small>{t("Rent, living costs & more")}</small></div><strong>{forecastReady ? money(forecast.monthlyExpenses) : '—'}</strong></div><div className="cash-row"><span className="icon-box outgoing"><Building2 /></span><div><p>{t("Estimated mortgage payments")}</p><small>{t("Principal and interest")}</small></div><strong>{money(forecast.mortgagePayments)}</strong></div><div className="cash-footer"><span>{t("Left after expenses")}</span><strong>{forecastReady ? money(forecast.forecast) : '—'}</strong></div><p className="footnote">{t("Yearly records are divided by 12. Linked plan spending is counted within its plan; other one-time records are excluded.")}</p></section></div>}{cashFlowSection && <EstimatedIncomeSources entries={current} currency={currency} month={forecastMonth}/>}{cashFlowSection && <ExpensePlans {...expensePlans} remove={removePlan} currencies={preferencesData.currencies} onSpend={spendFromPlan} onRetry={refreshRecords}/>}<section id="workspace-records" className="panel records"><div className="panel-title"><h2>{section === 'Overview' ? t("All records") : t(section)} <span className="count">{tableLoading ? '—' : formatNumber(totalRecords, locale, 0)}</span></h2><span>{t("Fetched prices where available")}</span></div><RecordFilters value={filters} onChange={setFilters} categories={planning.data.categories} kinds={sectionKey === 'debts' ? lendingRecordKinds : sectionKey === 'cashflow' ? [...income,...expenses] : kinds}/>{filtersActive&&planning.error&&<p role="alert" className="error">{t(planning.error)}</p>}{tableLoading ? <LoadingPlaceholder label={t("Loading records…")}/> : visible.length ? <div className="table-scroll"><table><thead><tr><th>{t("Name")}</th><th>{t("Category")}</th><th>{cashFlowSection ? t("Start / record date") : t("Date / due date")}</th>{cashFlowSection && <th>{t("Repeats")}</th>}<th>{cashFlowSection ? t("Amount per occurrence") : t("Value")}</th><th>{t("Actions")}</th></tr></thead><tbody>{visible.map(r => <tr key={r.id} style={{borderLeft: `3px solid ${categoryColor(r.kind)}`}}><td><div className="record-name"><span className="record-icon">{r.kind === 'Crypto' ? <Bitcoin /> : r.kind === 'Property' || r.kind === 'Mortgage' ? <Building2 /> : <Wallet />}</span><div><strong>{r.name}</strong>{r.kind === 'Mortgage' && (r.estimated_monthly_payment ?? 0) > 0 && <small>{t("Estimated payment: {amount}/month", { amount: money(r.estimated_monthly_payment!, r.currency) })}</small>}{r.expense_plan_id && <small>{t('Expense plan: {name}', { name: expensePlans.plans.find(plan => plan.id === r.expense_plan_id)?.name || r.name })}</small>}{r.mortgage_payment_id && <small>{t("Mortgage payment · Principal: {principal} · Interest: {interest}", { principal: money(Number(r.payment_principal), r.currency), interest: money(Number(r.payment_interest), r.currency) })}</small>}{['Business', 'Property'].includes(r.kind) && (r.estimated_monthly_income ?? 0) > 0 && <small>{t("Estimated income: {amount}/month", { amount: money(r.estimated_monthly_income!, r.currency) })}</small>}{r.kind === 'Business' && <small>{t("Ownership: {percentage}%", { percentage: formatNumber(r.ownership_percentage ?? 100, locale) })}</small>}{r.business_id && <small>{t("Business: {name}", { name: availableBusinesses.find(b => b.id === r.business_id)?.name || t("Business") })}</small>}{<small>{['Stock', 'Crypto'].includes(r.kind) ? t('{quantity} units · Gain/loss {amount}', { quantity: formatNumber(r.quantity, locale), amount: money((r.amount - r.cost) * r.quantity, r.currency) }) : r.frequency === 'Once' ? (r.rate ? t('{rate}% annual interest', { rate: formatNumber(r.rate, locale) }) : r.notes || t("One-time record")) : t(r.frequency)}</small>}{['Stock', 'Crypto'].includes(r.kind) && <><small>{t('{price} per unit', { price: formatMoney(r.amount, r.currency, locale, true) })}</small><small>{quoteLabel(r)}</small></>}</div></div></td><td><CategoryBadge kind={r.kind} label={t(r.kind)}/>{r.custom_category_id&&<CategoryBadge kind={r.custom_category_id} label={planning.data.categories.find(c=>c.id===r.custom_category_id)?.name??t('Custom category')}/>}</td><td className="muted">{r.kind === 'Money lent' ? <><div>{t("Lent: {date}", { date: formatDate(r.lent_date || '') })}</div><small>{r.date ? t("Due: {date}", { date: formatDate(r.date) }) : t("No due date")}</small></> : formatDate(r.date)}</td>{cashFlowSection && <td>{r.end_date&&<small className="muted">{t("Ends on {date}",{date:formatDate(r.end_date)})}</small>}<span className="badge">{r.frequency === 'Once' ? t("One time") : r.frequency === 'Monthly' ? t("Every month") : t("Every year")}</span></td>}<td className="amount">{money(value(r), r.currency)}</td><td><div className="row-actions">{[...income,...expenses].includes(r.kind)&&r.frequency!=='Once'&&!r.end_date&&<Button size="sm" variant="outline" onClick={()=>setStopping(rows.find(row=>row.id===r.id)||r)}>{t("Stop")}</Button>}{!demo && trackedKinds.includes(r.kind) && <Button size="sm" variant="outline" onClick={() => setTracking(rows.find(row => row.id === r.id) || r)}>{t("Tracker")}</Button>}{r.kind === 'Mortgage' && <Button size="sm" variant="outline" onClick={() => setPayingMortgage(rows.find(row => row.id === r.id) || r)}>{t("Record payment")}</Button>}{!r.operation_id && !r.mortgage_payment_id && !r.history_event_id && <><Button size="icon" variant="ghost" aria-label={t('Edit {name}', { name: r.name })} onClick={() => editRecord(r)}><Pencil size={15}/></Button><Button size="icon" variant="ghost" aria-label={t('Delete {name}', { name: r.name })} onClick={() => {setError('');setDeleting(r);}}><Trash2 size={15}/></Button></>}</div></td></tr>)}</tbody></table></div> : <div className="empty"><Wallet /><h3>{t("A fresh start.")}</h3><p>{t('Add a record in {currency} to start building your overview.', { currency })}</p>{section !== 'Overview' && <Button variant="outline" onClick={addRecord}><Plus />{cashFlowSection ? t("Add your first expense") : t("Add your first record")}</Button>}</div>}{!tableLoading && <nav className="records-pagination" aria-label={t('Record pages')}><span>{t('Page {page} of {pages} · {count} records', { page: formatNumber(tablePage, locale, 0), pages: formatNumber(pageCount, locale, 0), count: formatNumber(totalRecords, locale, 0) })}</span><div><Button variant="outline" disabled={tableLoading || busy || tablePage <= 1} onClick={() => setPageState({ key: paginationKey, page: tablePage - 1 })}>{t('Previous')}</Button><Button variant="outline" disabled={tableLoading || busy || tablePage >= pageCount} onClick={() => setPageState({ key: paginationKey, page: tablePage + 1 })}>{t('Next')}</Button></div></nav>}</section></>}<p className="bottom-note"><ShieldCheck size={14}/>{demo ? t("Sample data for exploring the app.") : t("Private records · Only visible to your account.")}</p></>}</div></main>
 <RecordDialog editing={editing} setEditing={setEditing} busy={busy} rows={planning.data.records.length ? planning.data.records : rows} save={save} editingCashFlow={editingCashFlow} recordKinds={recordKinds} demo={demo} summary={summary} field={field} linkedExpensePlan={linkedExpensePlan} preferencesData={preferencesData} availableBusinesses={availableBusinesses} expensePlans={expensePlans} money={money} fetchingPrice={fetchingPrice} fetchPrice={fetchPrice} priceMessage={priceMessage} error={error} planning={planning}/>
 {tracking && <InvestmentTracker key={tracking.id} record={tracking} accounts={planning.data.records} onClose={() => setTracking(null)} onSaved={refreshRecords} onPayment={() => { setPayingMortgage(rows.find(row => row.id === tracking.id) || tracking); setTracking(null); }}/>}
