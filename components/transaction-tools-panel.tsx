...
export function TransactionToolsPanel({tools,categories,saveCategory}:{tools:ToolsController;categories:Category[];saveCategory:(name:string)=>Promise<void>}){
 ...
 return <section id="categories" className="panel tools-panel"><h2>{t('Categories')}</h2><p className="muted">{t('Add categories here, then choose them when recording expenses.')}</p>
 ...
 <ul className="tool-list">{categories.map(item=><li key={item.id}><CategoryBadge kind={item.id} label={item.name}/></li>)}</ul>
