...
import { ArrowUpRight, ArrowDownLeft, GitCompareArrows, ChartNoAxesCombined, Wallet, ShieldCheck, LayoutDashboard, Landmark, HandCoins, Plus, LogOut, Pencil, Trash2, ChevronRight, Bitcoin, Building2, X, Settings } from 'lucide-react';
...
import { Entry, kinds, assets, liabilities, assetRecordKinds, lendingRecordKinds, income, expenses, value, estimatedCashFlow } from '@/lib/finance';
...
const sections = [['Overview', LayoutDashboard, '/'], ['Benchmarks', GitCompareArrows, '/benchmarks'], ['Assets & investments', ChartNoAxesCombined, '/assets'], ['Income & expenses', ArrowDownLeft, '/income-expenses'], ['Loans & debts', HandCoins, '/loans-debts'], ['Recently deleted', Trash2, '/recently-deleted'], ['Settings', Settings, '/settings']] as const;
...
function WorkspaceContent() {
    ...
    const visible = demo ? demoVisible.slice((page - 1) * 10, page * 10) : tableLoading ? [] : sectionKey === 'assets' ? sortAssetsByWorth(rows, r => marketEntry(r, currency, market)).slice((page - 1) * 10, page * 10).map(r => marketEntry(r, currency, market) ?? r) : rows.map(r => marketEntry(r, currency, market) ?? r);
