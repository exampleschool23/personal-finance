...
const schema=z.object({id:z.string().uuid(),name:z.string().trim().min(1).max(120),kind:z.enum(kinds),currency:z.string().refine(isCurrency),amount:z.number().finite().min(0).max(1e15),quantity:z.number().finite().min(0).max(1e12),cost:z.number().finite().min(0).max(1e15),rate:z.number().finite().min(0).max(1000),date:z.union([validDate,z.literal('')]),lent_date:z.union([validDate,z.literal('')]).optional(),end_date:validDate.nullable().optional(),frequency:z.enum(['Once','Monthly','Yearly']),estimated_monthly_payment:z.number().finite().min(0).max(1e15).default(0),estimated_monthly_income:z.number().finite().min(0).max(1e15).default(0),ownership_percentage:z.number().finite().min(0).max(100).default(100),business_id:z.string().uuid().nullable().optional(),expense_plan_id:z.string().uuid().nullable().optional(),notes:z.string().max(2000)}).superRefine((r,ctx)=>{if(r.end_date && (r.frequency==='Once' || ![...income,...expenses].includes(r.kind) || r.end_date<r.date))ctx.addIssue({code:'custom',path:['end_date'],message:'Check the end date.'});if(r.expense_plan_id && (!expenses.includes(r.kind) || r.frequency!=='Once' || r.business_id))ctx.addIssue({code:'custom',path:['expense_plan_id'],message:'Only one-time personal expenses can link to a plan.'});if(r.business_id && ![...income,...expenses].includes(r.kind))ctx.addIssue({code:'custom',path:['business_id'],message:'Only income and expenses can link to a business.'});if(r.kind==='Money lent'){if(!r.lent_date)ctx.addIssue({code:'custom',path:['lent_date'],message:'Date lent is required.'});if(r.date && r.lent_date && r.date<r.lent_date)ctx.addIssue({code:'custom',path:['date'],message:'Due date must not precede lending date.'});}else if(!r.date)ctx.addIssue({code:'custom',path:['date'],message:'Date is required.'});});
async function handle(req:Request,method:string){if(method!=='GET'&&!sameOrigin(req))return new Response(null,{status:403});try{const s=await session();if(!s)return Response.json({error:'Please sign in again.'},{status:401});let path='/rest/v1/finance_records';let init:RequestInit={};if(method==='GET'){
 ...
 const query=z.object({page:z.coerce.number().int().min(1).max(1000000),section:z.enum(['all','assets','cashflow','debts']),currency:z.string().refine(isCurrency).nullable(),summary:z.enum(['0','1'])}).safeParse({page:params.get('page')||'1',section:params.get('section')||'all',currency:params.get('currency'),summary:params.get('summary')||'0'});
...
if(method==='GET' && new URL(req.url).searchParams.get('section')==='assets'){
 ...
 for(let offset=0;;offset+=500){
  ...
  if(currency)params.set('currency','eq.'+currency);
