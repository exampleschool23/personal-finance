 ...
 return <>
  <section className="panel portfolio-trend">
   ...
   {loading ? <LoadingPlaceholder label={t('Loading history…')}/> : error ? <p role="alert" className="error">{t('Could not load portfolio history.')} <Button variant="outline" onClick={() => { setError(false); setHistory(null); setRetry(n => n + 1); }}>{t('Retry')}</Button></p> : <>
    ...
    <InvestmentComparison history={history??{records:[],events:[]}} today={today} currency={currency} market={market} demo={demo} embedded={{days:range??0,points:visible,tooltip:<PortfolioTooltip showBalanceDifference={false} valueLabel="Investment value" details={details} currency={currency} onOpenActivity={onOpenActivity}/>}}/>
