...
export function UpcomingPage({data,save}:{data:PlanningData;save:(action:string,data:unknown)=>Promise<void>}){
 ...
 <div className="table-scroll"><table><thead><tr><th>{t('Date')}</th><th>{t('Name')}</th><th>{t('Amount')}</th><th>{t('Status')}</th><th>{t('Actions')}</th></tr></thead><tbody>{section.items.map(item=><tr key={item.key}><td>{formatDate(item.date,locale)}</td><td>{item.record.name}<small className="block">{t(item.record.kind)}</small></td><td>{formatMoney(item.record.amount,item.record.currency,locale)}</td><td className={item.overdue?'negative':''}>{t(item.overdue?'Overdue':'Upcoming')}</td><td>{item.type==='maturity'?<Button disabled={busy||item.date>today} variant="outline" onClick={async()=>{setBusy(true);try{await save('dismiss',{id:crypto.randomUUID(),target_id:item.record.id,date:item.date});}catch(e){setError((e as Error).message);}finally{setBusy(false);}}}>{t('Dismiss reminder')}</Button>:<Button disabled={item.date>today} variant="outline" onClick={()=>setOperation({action:item.type==='scheduled'?'occurrence':item.record.kind==='Mortgage'?'mortgage':'repayment',target_id:item.record.id,date:item.type==='scheduled'?item.date:today,amount:item.type==='scheduled'?item.record.amount:0})}>{t('Record payment')}</Button>}</td></tr>)}</tbody></table></div>{!section.items.length&&<p className="muted">{t('No unpaid items in this period.')}</p>}
 </section>;
 })}</div>
