...
const get=apiFunction('z','kinds','session','supa','sameOrigin','isCurrency','income','expenses',js+';return GET;')(z,kinds,async()=>({user:{id:'owner'},token:'test-token'}),supa,()=>true,isCurrency,income,expenses);
...
test('rejects invalid pagination rather than issuing an unrestricted query',async()=>{
 for(const query of ['page=0','page=-1','page=1.5','page=1000001','section=bad','currency=ZZZ','summary=yes']){
