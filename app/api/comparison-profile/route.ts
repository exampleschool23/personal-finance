...
const preferences = z.object({benchmarks:z.array(z.enum(benchmarkKeys)).min(1).max(6).refine(items=>new Set(items).size===items.length),custom_symbol:z.string().regex(/^$|^[A-Z][A-Z0-9.-]{0,14}$/)}).refine(value=>!value.benchmarks.includes('CUSTOM')||!!value.custom_symbol);
...
export async function GET() {
 try {
  ...
  const [saved,capital]=await Promise.all([supa('/rest/v1/investment_comparison_preferences?select=benchmarks,custom_symbol',{},auth.token),supa('/rest/v1/investment_comparison_baselines?select=starting_amount,currency,capital_as_of,holdings',{},auth.token)]);
