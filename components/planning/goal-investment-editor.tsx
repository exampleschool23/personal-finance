...
import { isInstrumentAccount, type HoldingAccount } from '@/lib/holding-accounts';
...
export function GoalInvestmentEditor({targets,accounts,onChange,busy}:{targets:InvestmentTarget[];accounts:HoldingAccount[];onChange:(targets:InvestmentTarget[])=>void;busy:boolean}){
 ...
 const add=()=>{const account=accounts.filter(isInstrumentAccount)[0];if(account)onChange([...targets,{holding_account_id:account.id,asset_kind:account.kind,asset_symbol:'',target:0,monthly_contribution:null}]);};
 return <div className="investment-target-editor">
  ...
  {targets.map((target,index)=>{
   ...
   return <fieldset className="investment-target-row" key={index} disabled={busy}>
    ...
    <label>{t('Investment account')}<NativeSelect required value={target.holding_account_id} onChange={event=>{const account=accounts.filter(isInstrumentAccount).find(item=>item.id===event.target.value);if(account)update(index,{holding_account_id:account.id,asset_kind:account.kind,asset_symbol:'',target:0,monthly_contribution:null});}}><option value="">{t('Select account')}</option>{accounts.filter(isInstrumentAccount).map(account=><option key={account.id} value={account.id}>{account.name} · {t(account.kind==='Crypto'?'Crypto account':'Stock account')}</option>)}</NativeSelect></label>
