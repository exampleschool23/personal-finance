...
export function FormattedNumberInput({ value, onValueChange, max = 1e15, required = true }: { value: number; onValueChange: (value: number) => void; max?: number; required?: boolean }) {
  ...
  const [text, setText] = useState(() => numberInputValue(value, locale));
  const lastEmitted = useRef(value);
  const previousLocale = useRef(locale);
  useEffect(() => {
    if (value !== lastEmitted.current || locale !== previousLocale.current) setText(numberInputValue(value, locale));
    lastEmitted.current = value; previousLocale.current = locale;
  }, [value, locale]);
  return <Input type="text" inputMode="decimal" autoComplete="off" value={text} required={required} onChange={event => {
  ...
  }} onBlur={() => { if (text !== '') setText(numberInputValue(value, locale)); }} />;
}
