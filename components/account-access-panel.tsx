...
export function AccountAccessPanel({settings=false,tokenHash='',tokenType='email',onSignedOut}:{onSignedOut?:()=>void;settings?:boolean;tokenHash?:string;tokenType?:string}){
 ...
 return <section className="panel tools-panel"><h2>{t(settings?'Account security':'Account access')}</h2><p>{t('Use a unique password with at least 12 characters.')}</p>
 <div className="entry-actions">{(settings?['change_password',...(capabilities.deletion?['delete_account']:[])]:tokenHash?[]:['recover',...(capabilities.signup?['signup']:[])]).map(action=><Button type="button" key={action} variant={mode===action?'default':'outline'} onClick={()=>{setMode(action);setError('');setMessage('');}}>{t(({change_password:'Change password',delete_account:'Delete account',recover:'Forgot password',signup:'Create account'} as Record<string,string>)[action])}</Button>)}</div>
 ...
 </fieldset><Button disabled={busy||(needsPassword&&password!==repeat)||(mode==='delete_account'&&confirmation!=='DELETE')}>{t(busy?'Saving…':mode==='verify'?'Verify email link':mode==='delete_account'?'Permanently delete account':'Continue')}</Button>
