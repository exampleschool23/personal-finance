...
export function DebtSummary({entries,currency}:{entries:Entry[];currency:string}) {
 ...
 const debts=entries.filter(entry=>liabilities.includes(entry.kind));
 const receivable=lent.reduce((sum,entry)=>sum+value(entry),0);
 const payable=debts.reduce((sum,entry)=>sum+value(entry),0);
 ...
 return <div className="metrics debt-metrics">
  ...
  <article><p>{t('Net lending position')}<Scale size={19}/></p><h2>{formatMoney(receivable-payable,currency,locale)}</h2><small>{t('Money owed to you minus your outstanding debts.')}</small></article>
 </div>;
}
