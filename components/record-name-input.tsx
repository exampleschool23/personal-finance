...
export function RecordNameInput({ entry, rows, onChange, placeholder }: { entry: Entry; rows: Entry[]; onChange: (name: string) => void; placeholder: string }) {
  ...
  return <div className="record-name-field"><label htmlFor={id}>{t('Name')}</label><Input id={id} value={entry.name} onChange={e => { setSelection(null); onChange(e.target.value); }} maxLength={120} placeholder={placeholder} required autoComplete="off" aria-describedby={!dismissed && matches.length ? id + '-matches' : undefined}/>
