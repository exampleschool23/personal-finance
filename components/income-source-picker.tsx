...
export type IncomeSourceOption={id:string;name:string;kind:string;disabled?:boolean;estimate?:number|null;currency?:string;frequency?:string|null;payment?:{due:string;paid:boolean}|null};
export function IncomeSourcePicker({options,value,disabled,onChange}:{options:IncomeSourceOption[];value:string;disabled?:boolean;onChange:(id:string)=>void}){
 ...
 const matching=options.filter(item=>`${item.name} ${t(item.kind)}`.toLocaleLowerCase(locale).includes(query));
 const categories=income.filter(kind=>matching.some(item=>item.kind===kind));
 const active=category==='all'||categories.includes(category as typeof income[number])?category:'all';
  ...
  <PopoverContent align="start" className="income-source-popover" aria-labelledby={labelId}>
   ...
   <div className="income-source-columns"><div className="income-source-categories" aria-label={t('Category')}>
    {['all',...categories].map(kind=><button type="button" key={kind} aria-pressed={active===kind} aria-controls={resultsId} onMouseEnter={()=>setCategory(kind)} onFocus={()=>setCategory(kind)} onClick={()=>setCategory(kind)} onKeyDown={event=>{if(event.key==='ArrowRight'){event.preventDefault();results.current?.querySelector<HTMLButtonElement>('button:not(:disabled)')?.focus();}}}>
     <span>{kind==='all'?t('All income sources'):t(kind)}</span><small>{formatNumber(matching.filter(item=>kind==='all'||item.kind===kind).length,locale,0)}</small>
    </button>)}
    ...
    <p className="muted">{active==='all'?t('All income sources'):t(active)}</p>
    {visible.map(item=><button type="button" key={item.id} disabled={item.disabled} aria-pressed={value===item.id} onClick={()=>choose(item.id)}><span><span className="income-source-title-row"><strong>{item.name}</strong><CategoryBadge kind={item.kind} label={t(item.kind)}/></span>
