...
export function AssetDistribution({ entries, currency }: { entries: Entry[]; currency: Entry['currency'] }) {
  ...
  const slices = categories.map((group, index) => {
    const start = categories.slice(0, index).reduce((sum, item) => sum + item.amount, 0);
    return `${categoryColor(group.kind)} ${total ? start / total * 100 : 0}% ${total ? (start + group.amount) / total * 100 : 0}%`;
  });
