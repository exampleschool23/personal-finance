...
export const earningSourceSchema=z.object({
 ...
 amount:z.number().finite().nonnegative().max(1e15).nullable(),frequency:z.enum(['Monthly','Yearly']).nullable(),start_date:date.nullable(),end_date:date.nullable(),
...
export function resolveEarningSource(entry:Entry,sources:EarningSource[],original?:Entry):Partial<Entry>{
 ...
 if(patch.earning_due_on){
  ...
  if(due<source.start_date!||(source.end_date&&due>source.end_date)||salaryDueDate({date:source.start_date!,frequency:source.frequency!},due)!==due)throw Error('Choose a scheduled payment date.');
