...
export function InstrumentPicker({kind,value,onChange,disabled=false}:{kind:'Crypto'|'Stock';value:string;onChange:(value:string)=>void;disabled?:boolean}){
 const {t}=useLanguage();
 const id=useId();
 ...
 const matches=matchingInstruments(kind,query);
 const custom=kind==='Stock'&&!matches.length?customStockSymbol(query):null;
 const label=t(kind==='Crypto'?'Coin':'Stock or ETF (USD-listed)');
 function select(next:string){onChange(next);setOpen(false);setQuery('');}
 return <div className="instrument-field">
  <label id={id+'-label'} htmlFor={id}>{label}</label>
  <Popover open={open} onOpenChange={next=>{setOpen(next);if(!next)setQuery('');}}>
   <PopoverTrigger asChild><Button id={id} type="button" variant="outline" role="combobox" aria-expanded={open} aria-controls={open?id+'-list':undefined} aria-labelledby={id+'-label '+id+'-value'} disabled={disabled} className="instrument-trigger">
    <span id={id+'-value'}>{selected?`${selected.name} (${selected.symbol})`:value||t(kind==='Crypto'?'Select a coin':'Select a stock or ETF')}</span><ChevronsUpDown aria-hidden="true" size={16}/>
   ...
   <PopoverContent align="start" className="instrument-popover">
    <Command shouldFilter={false}>
     ...
     <CommandList id={id+'-list'} aria-label={label}>
      {!custom&&<CommandEmpty>{t('No matching instruments.')}</CommandEmpty>}
      ...
      {custom&&<CommandGroup heading={t('Other ticker')}><CommandItem value={'custom:'+custom} onSelect={()=>select(custom)}>{t('Use ticker {symbol}',{symbol:custom})}</CommandItem></CommandGroup>}
     </CommandList>
    </Command>
