...
export function SpendingWatchlists({data,splits,today,currencies,preferences}:{data:PlanningData;splits:TransactionSplit[];today:string;currencies:string[];preferences:PreferenceResource}){
 const {t,locale}=useLanguage();const lists=preferences.data.preferences.find(p=>p.key==='watchlists')?.data.items??[];const [name,setName]=useState(''),[query,setQuery]=useState(''),[category,setCategory]=useState(''),[currency,setCurrency]=useState(currencies[0]),[target,setTarget]=useState(0),[busy,setBusy]=useState(false),[error,setError]=useState('');
 ...
 <details><summary>{t('Add spending watchlist')}</summary><form onSubmit={async event=>{event.preventDefault();if(await save([...lists,{id:crypto.randomUUID(),name,query,category,currency,target}])){setName('');setQuery('');setTarget(0);}}}><fieldset disabled={busy} className="tracker-fields"><label>{t('Name')}<Input value={name} required maxLength={80} onChange={event=>setName(event.target.value)}/></label><label>{t('Merchant contains')}<Input value={query} maxLength={120} onChange={event=>setQuery(event.target.value)}/></label><label>{t('Category')}<NativeSelect value={category} onChange={event=>setCategory(event.target.value)}><option value="">{t('All categories')}</option>{data.categories.map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</NativeSelect></label><label>{t('Currency')}<NativeSelect value={currency} onChange={event=>setCurrency(event.target.value)}>{currencies.map(code=><option key={code}>{code}</option>)}</NativeSelect></label><label>{t('Monthly target')}<FormattedNumberInput value={target} onValueChange={setTarget}/></label></fieldset><Button disabled={busy||target<=0||!name.trim()||lists.length>=30}>{t('Save')}</Button></form></details></>}{error&&<p role="alert">{t(error)}</p>}</section>;
}
