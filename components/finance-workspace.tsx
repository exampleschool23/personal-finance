...
export default function FinanceWorkspace() {
    ...
    const { market, loading: marketLoading, error: marketError, refresh } = useMarket(rows, !!user || demo);
    ...
    async function load() { const r = await fetch('/api/records'); const d = await r.json() as Entry[] & {
        ...
        throw new Error(d.error); setRows(d.map((e: Entry) => ({ ...e, date: e.date ?? '', lent_date: e.lent_date ?? '', amount: Number(e.amount), quantity: Number(e.quantity), cost: Number(e.cost), rate: Number(e.rate) }))); }
    ...
    const current = rows.map(r => marketEntry(r, currency, market)).filter((r): r is Entry => r !== null), sum = (types: readonly string[]) => current.filter(r => types.includes(r.kind)).reduce((n, r) => n + value(r), 0), totalAssets = sum(assets), totalDebt = sum(liabilities), monthlyIncome = current.filter(r => income.includes(r.kind)).reduce((n, r) => n + monthly(r), 0), monthlyExpense = current.filter(r => expenses.includes(r.kind)).reduce((n, r) => n + monthly(r), 0);
