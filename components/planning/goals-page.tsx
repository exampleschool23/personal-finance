...
type Props={preferences:PreferenceResource;owner:string|null;demo:boolean;revision:number;onSaved:()=>void;data:PlanningData;save:(action:string,data:unknown)=>Promise<void>;currencies:string[];market:MarketData|null;plans:ExpensePlan[];plansReady:boolean;snapshots:PortfolioSnapshot[];historyError:string};
export function GoalsPage({preferences,owner,demo,revision,onSaved,data,save,currencies,market,plans,plansReady,snapshots,historyError}:Props){
 ...
 return <>
  ...
  <GoalFundingPanel data={data} currency={currencies[0]} surplus={totals.get(currencies[0])?.surplus??null} today={today} rates={market?.rates} owner={owner} demo={demo} revision={revision} onSaved={onSaved}/>
