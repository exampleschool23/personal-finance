...
Snapshots are observations, not historical market backfills. With the app closed, no new samples are collected; the line connects the days actually observed. Same-day refreshes replace today's observation rather than creating an intraday trading chart. Manual asset values change when updated in the app; quoted stock and crypto values change automatically when prices refresh.
