...
export function useExpensePlans(user:string|null,demo:boolean,rows:Entry[],reload:number,onSaved:()=>void) {
 const [saved,setSaved]=useState<ExpensePlan[]>([]),[demoPlans,setDemoPlans]=useState<ExpensePlan[]>([]);
 const [loading,setLoading]=useState(true),[error,setError]=useState('');
 const month=expensePlanMonth();
 useEffect(()=>{
 ...
 return {plans,month,loading:!demo&&loading,error,save,remove};
}
