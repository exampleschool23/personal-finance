...
import { xirr,timeWeightedEstimate } from '@/lib/portfolio-performance';
...
export function InvestmentComparison({history,today,currency,market,demo}:{history:History;today:string;currency:string;market:MarketData|null;demo:boolean}){
 ...
 const annualized=performance&&!performance.missing?xirr([...performance.flows.map(flow=>({...flow,amount:-flow.amount})),...performance.distributions,{date:today,amount:(performance.points.at(-1)?.amount??0)-performance.distributed}]):null;
 const weighted=performance&&!performance.missing?timeWeightedEstimate(performance.points,performance.flows):null;
  ...
  {demo?<p className="comparison-note">{t('Sign in to save investment comparisons.')}</p>:<>
   ...
   {!profile&&!profileError?<LoadingPlaceholder label={t('Loading comparisons…')}/>:<>
    ...
    {!events.length?<p className="comparison-notice">{t('Add an investment, then record the amount invested and its date in Tracker to compare it with Bitcoin.')}</p>:loadedKey!==requestKey?<LoadingPlaceholder label={t('Loading comparisons…')}/>:error?<p className="error" role="alert">{t(error)} <Button variant="outline" onClick={()=>{setLoadedKey('');setRetry(n=>n+1);}}>{t('Retry')}</Button></p>:<>
     ...
     {!!points.length&&<>
      <div className="review-grid"><article><h3>{t('Money-weighted return (annualized)')}</h3><strong>{annualized===null?'—':percent(annualized*100)}</strong></article><article><h3>{t('Time-weighted estimate (daily)')}</h3><strong>{weighted===null?'—':percent(weighted.total*100)}</strong></article><article><h3>{t('Estimated maximum drawdown')}</h3><strong>{weighted===null?'—':percent(weighted.drawdown*100)}</strong></article></div><p className="muted">{t('Annualized return is unavailable for ambiguous cash-flow signs. Daily time-weighted estimates assume end-of-day flows and use recorded valuations; they are not exact intraday returns.')}</p><div className="portfolio-headline"><div><span>{t('Total invested')}</span><strong>{money(Number(last?.invested??0))}</strong></div><div><span>{t('Investment gain')}</span><strong>{typeof lastValues?.actual==='number'?money(lastValues.actual-lastValues.contributed):'—'}</strong></div><div><span>{t('My return')}</span><strong>{typeof last?.actual==='number'?percent(last.actual):'—'}</strong></div></div>
