...
const sections = [['Overview', LayoutDashboard, '/'], ['Assets & investments', ChartNoAxesCombined, '/assets'], ['Income & expenses', ArrowDownLeft, '/income-expenses'], ['Loans & debts', HandCoins, '/loans-debts']] as const;
export default function FinanceWorkspace() {
    ...
    const [user, setUser] = useState<string | null>(null), [ready, setReady] = useState(false), [configured, setConfigured] = useState(true), [demo, setDemo] = useState(false), [rows, setRows] = useState<Entry[]>([]), [currency, setCurrency] = useState<'USD' | 'UZS'>('USD'), [editing, setEditing] = useState<Entry | null>(null), [deleting, setDeleting] = useState<Entry | null>(null), [busy, setBusy] = useState(false), [error, setError] = useState('');
    ...
    async function fetchPrice() {
        ...
        try {
            ...
            const amount = convertAmount(quote.usd, 'USD', target.currency, data.fx?.rate);
    ...
    const quoteLabel = (entry: Entry) => {
        ...
        if (!quote || (currency === 'UZS' && !market?.fx)) return t(market?.errors[instrumentKey(instrument)] || 'Saved price');
    ...
    const currencyFilter = market?.fx ? '' : currency;
    ...
    }, [user, demo, page, sectionKey, currencyFilter, reload]);
    ...
    } setUser(null); setDemo(false); setRows([]); setSummary([]); setRecordTotal(0); summaryLoaded.current = false; setError(''); }
    ...
    const visible = demo ? demoVisible.slice((page - 1) * 20, page * 20) : tableLoading ? [] : rows.map(r => marketEntry(r, currency, market)).filter((r): r is Entry => r !== null);
